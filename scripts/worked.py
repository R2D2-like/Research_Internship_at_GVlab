...
def perform_action(env, action, duration):
    """Performs the given action for the specified duration."""
...
class ExploratoryTask(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        # mujoco_arena = TableWithSpongeArena(
        #     table_full_size=self.table_full_size,
        #     table_friction=self.table_friction,
        #     table_offset=self.table_offset,
        #     sponge_size=self.sponge_size,
        #     sponge_friction=self.sponge_friction,
        # )
        ...
        # self.sponge = SpongeObject()
        # self.sponge = SpongeObject(name="sponge",friction=self.sponge_friction,damping=0.1)
    ...
    #     # Instead of using a placement initializer, directly set the sponge position
    #     # Let's assume we want to place the sponge at the center of the table with a known z offset
    #     sponge_xpos = 0.0  # Centered on the table in x
    #     sponge_ypos = 0.0  # Centered on the table in y
    #     sponge_zpos = self.table_offset[2] + self.sponge_size[2] / 2  # On top of the table, adjusting for sponge height
    #     self.initial_sponge_pos = np.array([sponge_xpos, sponge_ypos, sponge_zpos])

    #     # Set the initial position of the sponge
    #     # self.sim.model.body_pos[self.sim.model.body_name2id(self.sponge.root_body)] = self.initial_sponge_pos
...
print('sponge',env.sim.data.body_xpos[env.sponge_body_id])
...
# Set the initial action to zero
...
# initial_pos = np.array([0,0.5,0.8])
#end-effector orientation in the form of axis-angle coordinates (ax, ay, az), where the direction represents the axis and the magnitude represents the angle (in radians).

# initial_pos = np.array([0.0,1.57,-1.57,0,0,3.14])
#　スポンジの上面にエンドエフェクタを配置する
# initial_pos = sponge_pos + np.array([0, 0, sponge_size[2]])
# initial_orientation = np.array([0, 0, 0])

#ジョイントの初期位置とエンドエフェクタの初期姿勢を結合する


# init_qpos = np.concatenate([initial_pos, initial_orientation]) # shape: (6,)    

# シミュレーションをリセットし、初期位置にエンドエフェクタを移動する
env.reset()
# env.robots[0].set_robot_joint_positions(init_qpos)
# init_qpos（座標系）にエンドエフェクタの初期位置を設定する
# env.robots[0].set_robot_joint_positions([0.0,0,0.8,0,0,3.14])
# env.robots[0].controller.update_initial_joints([0.0,0,0.8,0,0,3.14])
...
env.robots[0].controller.set_goal(action=initial_action)
# env.robots[0].controller.set_goal(action=initial_action, set_pos=initial_pos, set_ori=initial_orientation)

def move_end_effector_to_position(env, target_position, threshold=0.001):
    """
    ...
    for _ in range(1000):  # 最大1000ステップ
        ...
        action[:3] = 15*displacement  # 変位に基づいてアクションを設定(15倍はなんとなく）
        ...
        if env.is_eef_touching_sponge():
            ...
            print('現在の位置',current_position)
            break
move_end_effector_to_position(env, initial_pos)
# 10秒間レンダリングしながら静止する
for _ in range(1000):
    env.render()
    time.sleep(0.01)

# move to the initial position (x, y, z) = (0, 0, 0.8)
action = np.zeros(env.action_dim)
action[2] = -0.8
perform_action(env, action, duration=2)
...
# Perform the first action: move in the normal direction at 0.01 m/s for 2 seconds
# The OSC_POSE controller's action space is [dx, dy, dz, droll, dpitch, dyaw, grip]
# Moving in the normal direction (z-axis in this case) at 0.01 m/s
action = np.zeros(env.action_dim)
action[2] = 0.001
perform_action(env, action, duration=2)

# Perform the second action: move laterally at 0.05 m/s for 1 second
action[2] = 0  # Stop moving in the normal direction
action[0] = 0.005  # Move horizontally (x-axis)
perform_action(env, action, duration=1)

# Perform the third action: move laterally at -0.05 m/s for 1 second
action[0] = -0.005  # Move horizontally in the opposite direction
perform_action(env, action, duration=1)
